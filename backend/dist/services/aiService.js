...
class AIService {
     ...
     */
    async analyzeBodyMeasurements(photoUrls, userHeight) {
        try {
     ...
     */
    async analyzeBodyWithPythonAI(photoUrls, userHeight) {
        try {
            console.log('Using Python AI service for body analysis');
            // Call Python AI service
            const response = await axios_1.default.post('http://localhost:8000/analyze/body', {
     ...
     */
    async analyzePoseEstimation(photoUrl) {
        try {
            // Using MediaPipe Pose or similar model via Hugging Face
            const response = await axios_1.default.post('https://api-inference.huggingface.co/models/google/movenet-singlepose-lightning', { inputs: photoUrl }, {
                headers: {
     ...
     */
    async analyzeBodyWithOpenAI(photoUrls, userHeight) {
        try {
     ...
     */
    async analyzeBodyProportions(photoUrls) {
        try {
     ...
     */
    calculateMeasurementsFromKeypoints(keypoints) {
        ...
        try {
            ...
            if (leftShoulder && rightShoulder) {
                ...
                const shoulderDistance = Math.sqrt(Math.pow(rightShoulder.x - leftShoulder.x, 2) +
                    Math.pow(rightShoulder.y - leftShoulder.y, 2));
                measurements.shoulders = Math.round(shoulderDistance * 0.4); // Rough conversion
            }
            if (leftHip && rightHip) {
                // Calculate hip width
                const hipDistance = Math.sqrt(Math.pow(rightHip.x - leftHip.x, 2) +
     ...
     */
    combineMeasurements(measurementsList) {
        const validMeasurements = measurementsList.filter(Boolean);
        ...
        const fields = ['chest', 'waist', 'hips', 'shoulders', 'inseam', 'armLength', 'neckCircumference', 'thighCircumference'];
     ...
     */
    generateMeasurementReasoning(measurements) {
        const parts = [];
        if (measurements.chest && measurements.waist && measurements.hips) {
            const waistToHipRatio = measurements.waist / measurements.hips;
            const chestToWaistRatio = measurements.chest / measurements.waist;
     ...
     */
    generateFitRecommendations(measurements) {
    ...
    async analyzeBodyType(photoUrl) {
        try {
            console.log('Analyzing body type with Python AI service');
            const response = await axios_1.default.post('http://localhost:8000/analyze/body-type', {
                image_url: photoUrl
            }, {
                timeout: 15000,
    ...
    async analyzeFaceShape(photoUrl) {
        try {
            console.log('Analyzing face shape with Python AI service');
            const response = await axios_1.default.post('http://localhost:8000/analyze/face-shape', {
                image_url: photoUrl
            }, {
                timeout: 15000,
                headers: { 'Content-Type': 'application/json' }
    ...
    async analyzeSkinTone(photoUrl) {
        try {
            console.log('Analyzing skin tone with Python AI service');
            const response = await axios_1.default.post('http://localhost:8000/analyze/skin-tone', {
                image_url: photoUrl
            }, {
                timeout: 15000,
                headers: { 'Content-Type': 'application/json' }
    ...
    async generateStyleSuggestions(input) {
        try {
    ...
    parseTextToStructuredResponse(text, input) {
    ...
    validateAndEnhanceResult(result, input) {
    ...
    enhanceOutfitDescription(outfit, input) {
        const enhancements = [];
        if (input.bodyType) {
    ...
    getDefaultColorsForOccasion(occasion) {
        const colorMap = {
    ...
    async generateOutfitImage(outfitDescription, userProfile) {
        try {
            const prompt = `
      ...
      `;
            ...
            while (result.status === 'starting' || result.status === 'processing') {
                ...
                const statusResponse = await axios_1.default.get(`https://api.replicate.com/v1/predictions/${predictionId}`, {
                    headers: {
                        'Authorization': `Token ${this.replicateApiToken}`
                    }
                });
    ...
    async pollReplicateResult(predictionId) {
        ...
        while (attempts < maxAttempts) {
            try {
                const response = await axios_1.default.get(`https://api.replicate.com/v1/predictions/${predictionId}`, {
                    headers: {
                        'Authorization': `Token ${this.replicateApiToken}`
                    }
                });
     ...
     */
    parseProportionAnalysis(result) {
