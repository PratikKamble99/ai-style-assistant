...
router.get('/:id', async (req, res, next) => {
    try {
        ...
        if (!trend) {
            throw (0, errorHandler_1.createError)('Trend not found', 404);
        }
        res.json({ trend });
    }
    catch (error) {
...
router.post('/:id/generate-outfit', async (req, res, next) => {
    try {
        const { id } = req.params;
        ...
        const suggestionData = {
            ...
            confidence: 0.88,
            products: {
                create: trend.products.slice(0, 4).map((product) => ({
        ...
        // Create suggestion
        const suggestion = await prisma_1.prisma.styleSuggestion.create({
            data: suggestionData,
            include: {
                outfits: true
            }
