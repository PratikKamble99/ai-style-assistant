...
router.get('/overview', (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    ...
    const userData = await dashboardService_1.dashboardService.getUserDashboardData(userId);
    const recommendations = await dashboardService_1.dashboardService.getPersonalizedRecommendations(userId);
    const dashboardData = {
        user: {
...
router.get('/metrics', (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    const userId = req.user?.id;
...
router.get('/analytics', (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    ...
    // Cache for 1 hour
    await cacheService_1.cacheService.set(cacheKey, analytics, 3600);
    res.json(analytics);
}));
...
router.get('/weather-suggestions', (0, asyncHandler_1.asyncHandler)(async (req, res) => {
...
// Track user activity
router.post('/activity', (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    const userId = req.user?.id;
    ...
    await dashboardService_1.dashboardService.trackUserActivity({
        userId,
        type,
        timestamp: new Date().toISOString(),
        metadata
...
router.get('/updates', (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    const userId = req.user?.id;
    const lastUpdate = req.query.since;
    if (!userId) {
        return res.status(401).json({ error: 'User not authenticated' });
    }
    // Get latest metrics
    const metrics = await cacheService_1.cacheService.get('dashboard:latest_metrics');
    // Get user's recent activity since last update
    const recentActivity = (await cacheService_1.cacheService.get(`dashboard:recent_activity:${userId}`)) || [];
    ...
    if (lastUpdate && Array.isArray(recentActivity)) {
        ...
        newActivity = recentActivity.filter((activity) => new Date(activity.timestamp) > sinceDate);
    }
    res.json({
        timestamp: new Date().toISOString(),
        metrics,
