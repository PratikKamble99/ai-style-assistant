...
class NotificationService {
  ...
  async sendTrendingOutfitsNotification(userId: string, trendingOutfits: any[]) {
    try {
      const user = await prisma.user.findUnique({
        where: { id: userId },
        include: { notificationPreferences: true },
  ...
  async sendBulkTrendingNotifications(trendingOutfits: any[]) {
    try {
      ...
      const promises = users.map(user => 
        this.sendTrendingOutfitsNotification(user.id, trendingOutfits)
      );
  ...
  private generateTrendingOutfitsEmail(userName: string, outfits: any[]): string {
    return `
      <!DOCTYPE html>
      <html>
      <head>
